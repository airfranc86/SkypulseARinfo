...
float lineFn(vec2 st, float width, float perc, float offset,
  ...
  float blurPx = u_line_blur * pixel(1.0, iResolution.xy) * blur;
  float line_start = smoothstep(y + (width/2.0) + blurPx, y, st.y);
  float line_end   = smoothstep(y, y - (width/2.0) - blurPx, st.y);
...
interface ThreadsProps {
  /** RGB color [0–1] for the thread lines. Default: brand gold #c09c2b */
...
export function Threads({
  color = [0.753, 0.612, 0.169],   // #c09c2b brand gold
  amplitude = 2,
  distance = 0.3,
